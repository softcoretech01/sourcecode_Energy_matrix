        ...
        {data.map((item: any) => (
            <SelectItem key={item.id} value={item.chargeName}>
                {item.chargeName}
                            ...
                            <Button size="sm" className="h-9 text-sm bg-emerald-600 hover:bg-emerald-700 text-white px-4" onClick={() => navigate(`${import.meta.env.VITE_BASE_URL}/master/consumption-charges/add`)}>
                                + New
                            </Button>
                           <Button
    size="sm"
...
>
  ...
  onClick={() => navigate(`${import.meta.env.VITE_BASE_URL}/master/consumption-charges/edit/${row.id}`)}
