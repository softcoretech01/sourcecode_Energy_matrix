...
useEffect(() => {
    ...
    chargeCode: item.charge_code, // ✅ correct
    chargeName: item.charge_name, // ✅ correct
    description: item.charge_description, // ✅ correct
    date: item.valid_upto || "-",
    status: item.is_submitted === 1 ? "Posted" : "Saved",
    is_submitted: item.is_submitted, // ✅ keep this
    id: item.id
}));
...
const handleDelete = async (id: number) => {
    ...
    try {
        await api.delete(`/consumption/delete/${id}`);
        fetchData();
    } catch (err) {
        console.error(err);
    }
...
function setData(formatted: any) {
