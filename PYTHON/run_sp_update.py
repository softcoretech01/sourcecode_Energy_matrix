...
try:
    cursor.execute("DROP PROCEDURE IF EXISTS sp_add_customer_se;")
    cursor.execute("""
...
END
""")
    cursor.execute("DROP PROCEDURE IF EXISTS sp_update_customer_se;")
    cursor.execute("""
...
""")
    conn.commit()
    print("Successfully updated stored procedures")
